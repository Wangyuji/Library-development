...
public class BookController extends AbstractController {
    ...
    private void processBookRequest(
        ...
        Category selectedCategory = setCategoryModel(categoryService, model, categoryId, true);
        List<Book> bookList;
        ...
        filterBooksByTitle(bookList, title, author, isAdmin);
     ...
     * @param isAdmin
     */
    private void filterBooksByTitle(List<Book> bookList, String title, String author, boolean isAdmin) {
        title = title == null ? null : title.toLowerCase();
        author = author == null ? null : author.toLowerCase();
        if (title == null && author == null && isAdmin) {
            return;
        }
        ...
        while (iterator.hasNext()) {
            ...
            if ((title != null && !book.getTitle().toLowerCase().contains(title))
                    || (author != null && !book.getAuthor().toLowerCase().contains(author))
                    || (!isAdmin && !book.isCanBeBorrowed())) {
                iterator.remove();
            }
        }
    }
